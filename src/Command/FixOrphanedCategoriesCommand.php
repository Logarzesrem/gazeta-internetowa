...
class FixOrphanedCategoriesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($articles as $article) {
            ...
            if (null !== $category) {
                $categoryExists = $this->entityManager->getRepository(\App\Entity\Category::class)->find($category->getId());
                if (null === $categoryExists) {
                    $io->text(sprintf('Fixing article "%s" (ID: %d) - removing orphaned category reference', $article->getTitle(), $article->getId()));
        ...
        if ($fixedCount > 0) {
            ...
            $io->success(sprintf('Fixed %d articles with orphaned category references', $fixedCount));
        } else {
