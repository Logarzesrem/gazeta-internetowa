...
class ArticleFixtureTest extends DatabaseTestCase
{
    ...
    public function testLoadWithDependencies(): void
    {
        ...
        $article1 = $articleRepository->findOneBy(['title' => 'Renesans winyli: Powrót czarnej płyty do łask']);
        $this->assertNotNull($article1);
        $this->assertEquals('Artykuł o powrocie popularności płyt winylowych wśród melomanów i kolekcjonerów.', $article1->getContent());
        $this->assertEquals($adminUser, $article1->getAuthor());

        $article2 = $articleRepository->findOneBy(['title' => 'Sztuczna inteligencja w codziennym życiu: Jak AI zmienia nasze nawyki?']);
        $this->assertNotNull($article2);
        $this->assertEquals('Analiza wpływu sztucznej inteligencji na nasze codzienne życie i zmieniające się nawyki.', $article2->getContent());
        $this->assertEquals($adminUser, $article2->getAuthor());

        $article3 = $articleRepository->findOneBy(['title' => 'Zamknięcie mostu na trzy tygodnie. Objazdy już wyznaczone']);
        $this->assertNotNull($article3);
        $this->assertEquals('Informacje o planowanym zamknięciu mostu i wyznaczonych objazdach dla kierowców.', $article3->getContent());

        $article4 = $articleRepository->findOneBy(['title' => 'Teatr w wirtualnej rzeczywistości: Sztuka w epoce VR']);
        $this->assertNotNull($article4);
        $this->assertEquals('Eksploracja nowych możliwości teatru dzięki technologiom wirtualnej rzeczywistości.', $article4->getContent());
    }
    ...
    public function testLoadClearsExistingArticles(): void
    {
        ...
        $articleRepository = $this->entityManager->getRepository(Article::class);
        $this->assertNotNull($articleRepository->findOneBy(['title' => 'Existing Article']));
        ...
        $this->assertNull($articleRepository->findOneBy(['title' => 'Existing Article']));
    }
}
