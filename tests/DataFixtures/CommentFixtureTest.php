...
class CommentFixtureTest extends AbstractDatabaseTestCase
{
    ...
    public function testLoadWithDependencies(): void
    {
        ...
        $comment1 = $commentRepository->findOneBy(['content' => 'Świetny artykuł! Bardzo pouczające informacje o technologii.']);
        $this->assertNotNull($comment1);

        $comment2 = $commentRepository->findOneBy(['content' => 'Dziękuję za ten artykuł. Pomógł mi zrozumieć wiele kwestii.']);
        $this->assertNotNull($comment2);

        $comment3 = $commentRepository->findOneBy(['content' => 'Interesujący punkt widzenia. Czekam na więcej takich artykułów.']);
        $this->assertNotNull($comment3);

        $comment4 = $commentRepository->findOneBy(['content' => 'Bardzo dobrze napisane. Polecam wszystkim zainteresowanym tematem.']);
